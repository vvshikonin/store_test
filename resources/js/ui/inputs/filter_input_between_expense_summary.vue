<template>
    <div class="mb-2 filter">
        ...
        <div v-if="mode == 1" class="d-flex flex-row align-items-center justify-content-between">
            <Input @input="setStart($event.target.value)" :value="values.start" :type="type" :step="step"
                :placeholder="null" class="filter-input form-control me-2" />
            <span class="text-muted">-</span>
            <Input @input="setEnd($event.target.value)" :value="values.end" :type="type" :step="step"
                :placeholder="null" class="filter-input form-control ms-2" />
        </div>
        <div v-else-if="mode == 2" class="d-flex flex-row align-items-center justify-content-between">
            <Input @input="setEqual($event.target.value)" :value="values.equal" :type="type" :step="step"
