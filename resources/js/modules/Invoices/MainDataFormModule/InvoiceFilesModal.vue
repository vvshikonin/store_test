<template>
    <DefaultModal title="Файлы счёта" width="800px" modalStyle="align-self:self-start!important; margin-top:50px;">
        <div class="w-100 p-3">
            ...
            <div v-if="invoice.files.new_invoice_files.length">
                ...
                <ul class="text-primary p-3 pt-1 pb-1" style="max-height: 20dvh; overflow-y: auto;">
                    <li v-for="newFile, index in invoice.files.new_invoice_files" class="d-flex align-items-center">
            ...
            <div v-if="invoice.invoice_files.length">
                <small class="ps-3 text-muted">Сохранённые файлы:</small>
                <ul class="text-primary p-3 pt-1 pb-1" style="max-height: 20dvh; overflow-y: auto;">
                    <li v-for="file, index in invoice.invoice_files" class="d-flex align-items-center">
...
export default {
    ...
    methods: {
        ...
        addFilesToPreview(files) {
            for (let i = 0; i < files.length; i++) {
                if (!this.fileExists(files[i])) {
                    this.$store.commit('invoiceModule/addNewInvoiceFile', files[i])
                }
            }
        },
        fileExists(file) {
            return this.invoice.files.new_invoice_files.some(f => f.name === file.name);
        },
        removeFile(index) {
            this.invoice.files.new_invoice_files.splice(index, 1);
        },
        dragEnter() {
        ...
        removeSavedFile(index) {
            const file = this.invoice.invoice_files[index];
            this.$store.commit('invoiceModule/addDeletedInvoiceFile', file);
            this.invoice.invoice_files.splice(index, 1);
        }
    }
}
</script>